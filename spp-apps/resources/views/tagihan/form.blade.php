                ...
                <div class="card-body">
                    ...
                    <div class="row">
                        <div class="col-12">
                            ...
                            {{-- <div class="form-group">
                                <label class="form-label">Nama</label>
                            <select id="select-beast" class="form-control custom-select" name="siswa_id">
                                @foreach($siswa as $item)
                                <option value="{{ $item->id }}">{{ $item->nama }}</option>
                                    <option value="{{ $item->id }}" {{ isset($siswa) ? : '' }}>{{ $item->nama_kelas }} - {{ isset($item->periode) ? $item->periode->nama : '' }}</option>
                                @endforeach
                            </select>
                            </div> --}}
                            <div class="form-group">
                                ...
                                <select class="form-control" name="nama">
                                    <option value="#">[-- Pilih Siswa --]</option>
                                    @foreach($siswa as $item)
                                        <option value="{{ $item->nama }}"> {{ $item->nama.' - '.$item->kelas->nama_kelas.' - ' }}  DSP </option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="form-group">
                                <label class="form-label">Jumlah</label>
                                <input type="text" class="form-control" name="jumlah" value="{{ isset($tagihan) ? $tagihan->jumlah : old('jumlah') }}" required>
                            </div>
                            <div class="form-group">
                                ...
                                <select class="form-control" name="kelas_id" id="hanya-kelas">
                                    @foreach($kelas as $item)
                                        <option value="{{ $item->id }}" {{ isset($tagihan) ? (($tagihan->kelas_id == $item->id) ? 'selected' : '') : '' }}>
                                            {{-- {{ $item->nama_kelas }} - {{ isset($item->periode) ? $item->periode->nama : '' }} --}}
                                            {{ $item->nama_kelas }}
                                        </option>
                                    @endforeach
                                </select>
